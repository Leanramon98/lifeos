...
export default function WorkspaceNotasPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = use(params);
  const { workspace } = useWorkspace(resolvedParams.slug);
  const { notes, pinnedNotes, isLoading, deleteNote, togglePinNote, archiveNote } = useNotes({ workspaceId: workspace?.id });
    ...
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-medium text-foreground">
          Notas <span className="text-foreground-tertiary ml-2 font-normal">{notes.length + pinnedNotes.length} documentos</span>
        </h3>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input 
              placeholder="Buscar en este workspace..." 
              className="pl-9 h-9"
              value={searchQuery}
            ...
            <section>
              ...
              {filteredNotes.others.length === 0 && filteredNotes.pinned.length === 0 && (
                <div className="h-48 flex flex-col items-center justify-center text-center border-2 border-dashed border-border rounded-2xl">
                  <p className="text-foreground-tertiary">No hay notas en este workspace</p>
                </div>
              )}
            </section>
          </div>
